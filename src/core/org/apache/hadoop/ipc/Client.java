...
public class Client {
	...
	/** modified by lynus */
	private Hashtable<ConnectionId, RdmaConnection> rdmaConnections;
	private boolean useRDMA = false;
	/** lynus */
	...
	private class Connection extends Thread {
		...
		private synchronized boolean waitForWork() {
			...
			if (!calls.isEmpty() && !shouldCloseConnection.get() && running.get()) {
				...
				return false;
			} else if (calls.isEmpty()) { // idle connection closed or stopped 没有被标记为即将关闭，但是call为空
				markClosed(null);
				return false;
	...
	/** modified by lynus */
	private class RdmaConnection extends Thread {
		...
		public RdmaConnection(ConnectionId remoteId) throws IOException {
			this.remoteId = remoteId;
			this.server = remoteId.getAddress();
			this.maxIdleTime = remoteId.getMaxIdleTime(); //10sec
			this.rpcTimeout = 5; //手动设为网络IO等待时间为5秒
			//	    	this.connectionRetryPolicy = remoteId.connectionRetryPolicy;
			//	    	this.pingInterval = remoteId.getPingInterval();
			Class<?> protocol = remoteId.getProtocol();
			LOG.info("RPC: use protocol=" + protocol.getCanonicalName());

			authMethod = AuthMethod.SIMPLE;
			UserGroupInformation ticket = remoteId.getTicket();
			header = new ConnectionHeader(protocol == null?null : protocol
					.getName(), ticket, authMethod);
			this.setName("IPC Client (" + socketFactory.hashCode() +")  RDMA connection to " +
					remoteId.getAddress().toString() +
					" from " + ((ticket==null)?"an unknown user":ticket.getUserName()));
			this.setDaemon(true);
		...
		private class SessionCallback implements ClientSession.Callbacks {
			//note 可以访问 外围Client
			...
			public void onResponse(Msg msg){
				int id = 0;
				if (shouldCloseConnection.get()) {
					LOG.error("don't know why at this point shouldCloseConnect is true");
				}
				touch();
				LOG.info("onResponse event caught!");
				try {
					DataInputBuffer d = new DataInputBuffer();
					byte array[] = new byte[msg.getIn().limit()];
					msg.getIn().get(array);
					d.reset(array, array.length);
					id = d.readInt();
					LOG.info("got msg " + id);
					Call call = calls.get(id);
					int state = d.readInt();
					if (state == Status.SUCCESS.state) {
						Writable value = ReflectionUtils.newInstance(valueClass, conf);
						value.readFields(d);                 // read value
						call.setValue(value);	  		          
					} else if (state == Status.ERROR.state) {
						call.setException(new RemoteException(WritableUtils.readString(d),
								WritableUtils.readString(d)));	  			      
					} else if(state == Status.FATAL.state) {
						markClosed(new RemoteException(WritableUtils.readString(d), 
								WritableUtils.readString(d)));
					}
				}catch (IOException e) {
					markClosed(e);
			...
			public void onSessionEvent(EventName event, EventReason reason) {
				if (event == EventName.SESSION_CLOSED) {
					LOG.info("server ask to close");
					markClosed(null);
				}else if (event == EventName.SESSION_ERROR) {
					LOG.info("connection error");
					markClosed(new IOException());
				}else {
					LOG.info("server refused");
					markClosed(new ConnectException("server refused"));
				}
				return;
			}
			public void onMsgError(Msg msg, EventReason reason) {
				LOG.info("msg send/receive error");
				msg.returnToParentPool();
				markClosed(new IOException("msg send/recv error"));
				return;
			}
		}
		...
		private void setupSession() {
			...
			try {
				...
				uri = new URI(trans + "://" + server.getAddress().getHostAddress()
						+":" + server.getPort() + "/");
			...
			mp = new MsgPool(128, 256, 256);
			session = new ClientSession(eqh, uri, new SessionCallback());

		}
		private void setup() {
			// 设置session，建立链接，发送rpcheader，启动接收线程
			if (eqh != null || shouldCloseConnection.get())
				return;
			setupSession();
			try {
				...
				if (event == 0 || event == -1 || shouldCloseConnection.get()) {
					LOG.info("rdma connection hand shake failed " 
							+ (event == 0?"time out":"caught exception"));
					if (event == 0)
						throw(closeException == null? new IOException("hand shake failed"):closeException);
					else
						throw(closeException == null? new IOException("server refused") : closeException);
				}
				touch();
				start();
				return;
				...
				if (t instanceof IOException)
					...
					markClosed(new IOException("Couldn't set up RDMA connection",t));
				LOG.info("RdmaConnection.setup failed");
				close();
			}
		}
		private synchronized boolean addCall(Call call) {
		...
		private synchronized void close() {
			...
			synchronized(rdmaConnections) {
				if (rdmaConnections.get(remoteId) == this) {
					rdmaConnections.remove(remoteId);
				}
			}
			...
			if (closeException == null) {
				...
				LOG.info("closing ipc connection to " + server + ": " +
						closeException.getMessage(),closeException);
			cleanupCalls();
			LOG.info(getName() + "rdma connection closed");
		}
		private void cleanupCalls() {
			Iterator<Entry<Integer, Call>> itor = calls.entrySet().iterator() ;
			while (itor.hasNext()) {
		...
		public void run() {
			int processedEvent;
			LOG.info(getName() + "rdma connection receiver start");
			while (waitForWork()) {
				processedEvent = eqh.runEventLoop(1, -1);
				boolean err = eqh.didExceptionOccur();
				if (err) {
					Exception ex = eqh.getCaughtException();
					ex.printStackTrace();
				}

				if (processedEvent == 0) {
					markClosed(new SocketTimeoutException("rdma trans time out") );
					break;
				}
				else if(processedEvent == -1) {
					markClosed(new IOException("unknown exception"));
					break;
				}
				//处理了一个事件，不一定是接收数据，可能是onError, 或者是onSessionClose
				//等到下一次waitForWork()再检查是否需要继续读取数据
			}
			close();
			LOG.info(getName() + " rdma receiver closed, remaining receiver "
					+ rdmaConnections.size());
		}
		public void sendParam(Call call) {
			Msg msg = null;
			if (shouldCloseConnection.get()) 
			...
			try {
				...
				msg.getOut().put(out.getData());
				byte array[] = new byte [out.getLength() + 4];
	...
	public Client(Class<? extends Writable> valueClass, Configuration conf, 
		...
		/** modified by lynus */
		this.useRDMA = conf.getBoolean(CommonConfigurationKeys.IPC_USE_RDMA_KEY, 
				CommonConfigurationKeys.IPC_USE_RDMA_DEFAULT);
		if (useRDMA)
			rdmaConnections = new Hashtable<ConnectionId, RdmaConnection>();
		else
			connections = new Hashtable<ConnectionId, Connection>();
		/** lynus */
	}
	...
	public void stop() {
		...
		if (!useRDMA) {        
			...
			synchronized (rdmaConnections) {
				for(RdmaConnection conn : rdmaConnections.values())
					conn.interrupt();
			}
			while(!rdmaConnections.isEmpty()) {
				try {
					Thread.sleep(300);
				} catch (InterruptedException e ) {}
			}
		...
		synchronized (call) {
			...
			if (call.error != null) {
				if (call.error instanceof RemoteException) {
					...
					// the remoteId
					throw wrapException(remoteId.address, call.error);
				}
			} else {
				return call.value;
			}
	...
	private RdmaConnection getRdmaConnection(ConnectionId remoteId,
		...
		do {
			synchronized (rdmaConnections) {
				connection = rdmaConnections.get(remoteId);
				if (connection == null) {
					connection = new RdmaConnection(remoteId);
					rdmaConnections.put(remoteId, connection);
				}
			}
		} while (!connection.addCall(call));
		connection.setup();
